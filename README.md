...
使用 `requests` 呼叫 CWA Open Data API（資料集 `F-D0047-091`），取得台灣 22 縣市**未來一週**天氣預報 JSON 資料，並儲存至本地 `weather_data.json`。

- API 端點：`https://opendata.cwa.gov.tw/api/v1/rest/datastore/F-D0047-091`
- 每筆資料包含：最高溫度、最低溫度、平均溫度、相對濕度、降雨機率、天氣現象等
- 共 14 個時間段（每段 12 小時，日 / 夜各一），涵蓋未來約 7 天
...
- 將每日 12 小時的日 / 夜時段彙整為「每日」最高 / 最低溫
- 輸出結構化資料存至 `temperatures.json`（22 縣市 × 7 天 = 154 筆）
...
| dataDate | TEXT | 日期（YYYY-MM-DD） |
| mint | INTEGER | 最低氣溫（°C） |
| maxt | INTEGER | 最高氣溫（°C） |

執行後以三道 SQL 查詢驗證寫入結果：縣市列表、中部地區詳細資料、各縣市筆數。
...
**資料來源：** [中央氣象署開放資料平台](https://opendata.cwa.gov.tw)（資料集 F-D0047-091）
